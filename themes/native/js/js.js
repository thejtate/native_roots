(function ($) {
  ...
  function init() {
    initShortys($('.content-middle .field-collection-container .items-wrapper:nth-child(2) .item:nth-child(1) .field-item'));
  ...
  function initShortys(appendTo) {
    var $btn = $('.link-to-popup');

    if(!$btn.length || $btn.hasClass('loaded')) return;
  ...
  function initFlexslider() {
    $('#carousel').flexslider({
      ...
      start: function(slider) {
        slider.slides.each(function(index, val) {
          thumb($(val).find('img')[0], 129, 86);
        });
      }
    });
    ...
    $slider.flexslider({
      ...
      start: function(slider) {
        slider.slides.each(function(index, val) {
          thumb($(val).find('img')[0], slider.width(), slider.height());
        });
      }
    });
    ...
    $(window).on('resize', function() {
      $slides.each(function(index, val) {
        thumb($(val).find('img')[0], $slides.width(), $slides.height());
      });
    });

    function thumb(img, width, height) {
      var ratio = width / height < img.width / img.height;
